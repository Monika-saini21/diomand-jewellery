...
function Navbar() {
    ...
    return (
     ...
     <Link to= {"/wishlist"}> 
      ...
      (<p className='absolute top-4 right-27 text-white  bg-red-600  w-4 pb-4 pl-1 h-3   text-xs rounded-2xl '> 
      {wishListItem.length}
      </p>
      )
    } </Link>
     ...
     {cartItem.length>0 &&
    ( <p className='absolute top-4 right-17 text-white  bg-red-600  w-4 pb-4 pl-1 h-3   text-xs rounded-2xl'>
        {cartItem.length}
        </p>
        )}  </Link>
